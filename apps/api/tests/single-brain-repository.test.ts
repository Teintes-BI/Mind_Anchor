...
describe("Single brain SQLite migration", () => {
  it("creates the conversation, message, and context-packet tables", () => {
    ...
    expect(
      ...
      { name: "core_conversation_context_packets" },
  ...
  it("cascades conversation records when their core profile is deleted", () => {
    ...
    db.prepare("INSERT INTO core_conversations(id,profile_id,user_id,status,created_at,updated_at) VALUES ('conversation-1','profile-1','user-1','active',?,?)").run(now, now);
    ...
    db.prepare("INSERT INTO core_conversation_context_packets(id,conversation_id,profile_id,user_id,data_boundary,payload_json,context_hash,redactions_json,created_at) VALUES ('packet-1','conversation-1','profile-1','user-1','local_only','{}','sha256:packet-1','[]',?)").run(now);
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM core_conversation_context_packets").get()).toEqual({ count: 0 });
    db.close();
  });
});
