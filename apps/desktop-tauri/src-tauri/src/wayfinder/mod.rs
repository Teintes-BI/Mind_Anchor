...
impl SituationList {
    /// The situation to work on: the newest one still awaiting confirmation.
    ///
    /// Newest rather than first, because the server's order is not specified and
    /// confirming a stale situation is not what the user just asked for.
    pub fn active(&self) -> Option<&Situation> {
        self.situations
            .iter()
            .rfind(|situation| situation.awaits_confirmation())
    }
}
...
mod tests {
    ...
    #[test]
    fn the_active_situation_is_the_newest_awaiting_one() {
        let older = REAL_SITUATION.replace("1a854970-0ace-4422-8943-5b48fd3b1d8b", "older");
        let done = REAL_SITUATION
            .replace("1a854970-0ace-4422-8943-5b48fd3b1d8b", "done")
            .replace(
                r#""status": "awaiting_confirmation""#,
                r#""status": "confirmed""#,
            );
        let list: SituationList =
            serde_json::from_str(&format!(r#"{{"situations":[{older},{done}]}}"#)).unwrap();
        // The confirmed one must be skipped even though it comes later.
        assert_eq!(list.active().map(|s| s.id.as_str()), Some("older"));
    }

    #[test]
    fn no_awaiting_situation_means_nothing_active() {
    ...
    fn the_confirm_and_decision_bodies_carry_their_identifiers() {
        ...
        assert_eq!(decision["actionStatus"], "planned");
    }
